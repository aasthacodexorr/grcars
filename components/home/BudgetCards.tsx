...
import budgetGreatImg from "@/assets/cars/budget-great-deals-full.png"
...
export default function BudgetCards() {
    ...
    // 1. Reel Slot-Machine Animation Logic
    useEffect(() => {
        const root = priceRef.current;
        if (!root) return;

        root.innerHTML = ""; // Clear existing DOM on re-render
        const reels: { reel: HTMLDivElement; strip: HTMLDivElement }[] = [];

        const digitH = 52;

        const makeSym = (ch: string) => {
            const s = document.createElement("span");
            s.className = "block h-[52px] leading-[52px]";
            s.textContent = ch;
            return s;
        };
        ...
        const makeReel = () => {
            ...
            reel.className = "relative w-[0.62em] h-[52px] overflow-hidden";
            const strip = document.createElement("div");
            strip.className = "flex flex-col will-change-transform";
        ...
        const fillStrip = (strip: HTMLDivElement, startDigit: number) => {
            strip.innerHTML = "";
            for (let i = 0; i < 40; i++) {
                const span = document.createElement("span");
                span.className = "block h-[52px] leading-[52px] text-center";
                span.textContent = String((startDigit + i) % 10);
        ...
        const spinReel = (
            ...
            return new Promise<void>((resolve) => {
                fillStrip(strip, startDigit);
                const cycles = 2 + Math.floor(Math.random() * 2);
                const steps = cycles * 10 + ((finalDigit - startDigit + 10) % 10);

                setTimeout(() => {
                    const start = performance.now();
                    const to = steps * digitH;

                    function frame(now: number) {
                        const t = Math.min(1, (now - start) / duration);
                        strip.style.transform = `translateY(-${to * easeOutCubic(t)}px)`;
                        if (t < 1) {
                            requestAnimationFrame(frame);
                        } else {
                            fillStrip(strip, finalDigit);
                            resolve();
                        }
                    }
                    requestAnimationFrame(frame);
        ...
        root.appendChild(makeSym("$"));
        root.appendChild(makeReel());
        root.appendChild(makeReel());
        root.appendChild(makeSym(","));
        root.appendChild(makeReel());
        root.appendChild(makeReel());
                    ...
                    <div className="flex flex-row gap-5 items-stretch justify-start">
                        ...
                        <article className="card-item snap-center w-[300px] flex-none h-[340px] rounded-[18px] p-4.5 flex flex-col shadow-[0_12px_28px_rgba(0,44,95,0.1)] bg-[#a8d4fc]">
                            ...
                            <div className="relative h-[140px] my-2 overflow-visible" aria-hidden="true">
                                <div
                                    ref={priceRef}
                                    className="absolute left-1/2 top-[2px] -translate-x-1/2 z-[1] flex items-start text-[#0062bd] text-[52px] font-extrabold tracking-[-0.045em] leading-none tabular-nums select-none pointer-events-none"
                                />
                                <Image
            ...
            <div className="flex min-[900px]:hidden justify-center items-center gap-2 mt-1.5 pb-1">
                {[0, 1, 2, 3].map((i) => (
                    <button
                        ...
                        className={`w-2 h-2 rounded-full border-0 p-0 transition-all cursor-pointer ${activeIndex === i ? "bg-[#0062bd] scale-100" : "bg-[rgba(0,44,95,0.22)]"
                            }`}
                    />
                ))}
            </div>
